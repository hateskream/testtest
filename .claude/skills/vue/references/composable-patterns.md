...
**CRITICAL PRINCIPLE:** Start with inline code for component-specific logic. Only extract to external files when the
...
<script setup
lang = "ts" >
// Inline logic - used only in this component
...
// src/shared/composables/useHiddenFolders.ts - only used in one place!
...
|---------|-------------|---------|
| **Inline code** | Logic specific to ONE component | Form validation, local UI state, component-specific data fetching |
...
<script setup lang="ts">
	...
	import {useNetworkState} from '@/shared/composables/use-network-state'
	...
	// Inline logic #1: Hidden folders management
	...
	// Inline logic #2: Favorite folders management
...
Extract an inline logic to `src/shared/composables/` when:
...
// src/shared/composables/use-hidden-folders.ts
export function useHiddenFolders() {
	const showHiddenFolders = ref(
		localStorage.getItem('show-hidden-folders') === 'true'
	)
...
// src/shared/composables/use-component-specific-thing.ts - only used once!
...
- [ ] Is this logic used in only ONE component? → Inline logic
- [ ] Does it organize complex component logic? → Inline logic
...
**Note:** This section applies to **external composables** in `src/shared/composables/`. Inline logic (defined within
components) don't need separate files.
...
src/shared/composables/
  use-counter.ts        // External - reused in multiple components
  use-user-sata.ts      // External - shared business logic
  use-api-request.ts    // External - used across app
...
- Place in `src/shared/composables/` directory
...
## Reactive State Passing

Use `MaybeRefOrGetter` for support reactive state, `toValue` for get value from ref or getter
Use plain value if you sure that value will not change

```ts
export function useUserData(userId: MaybeRefOrGetter<string>) {
	const user = ref(null)

	watchEffect(() => {
		user.value = fetchUser(toValue(userId))
	})
...
// ✅ CORRECT: plain value if sure that value will not change
useUserData(toValue(props.));
```
...
Maintain ordering by logic within composable files:
...
- [ ] Located in `src/shared/composables/` directory or `/src/modules/name/composables`
