...
<template>
	<span :class="classes.wrapper">
		<template v-for="image in srcImage" :key="image">
			<span v-if="image && ticker" :class="classes.item">
				<ticker-icon
					:src="image"
					:ticker="ticker"
					:domain="srcImage?.[1]!"
...
.item:first-child {
	mask: radial-gradient(circle 15px at right 50%, transparent 0, transparent 13px, #ffffff 13px);
}
