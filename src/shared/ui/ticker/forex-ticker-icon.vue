...
const iconsSize = computed(() => {
	return props.size ? `${props.size}px` : '28px';
});
</script>
...
<template>
	...
	>
		...
		>
			<template #error>
				<icon-placeholder
					:class="[classes.firstIcon, classes.error]"
					:ticker="props.ticker"
		...
		>
			<template #error>
				<icon-placeholder
					:class="[classes.secondIcon, classes.error]"
					:ticker="props.domain"
					:style="{ width: iconsSize, height: iconsSize }"
				/>
			</template>
...
.secondIcon {
	...
	border-radius: 999px;
	outline: 1px solid var(--border-color-base-300);
}
