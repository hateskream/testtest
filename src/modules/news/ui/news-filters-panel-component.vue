...
import { SignalIcon } from '@/shared/ui/signal';

import NewsFilters from './news-filters-component.vue';
import NewsSegmentModal from './news-segment-modal.vue';
					...
					<ui-modal-badge-icon>
						<template #title>
							<div :class="classes.badgeIcon">
								<span
									v-for="score in selectedScores.values()"
									:key="score"
									:class="classes.iconBorder"
									:style="`color: ${getMetricIconColor(score)}`"
								>
									<signal-icon
										:class="classes.icon"
										:active="getScoreLevelActiveNumber(score)"
									/>
								</span>
							</div>
						</template>
				...
				>
					<ui-modal-badge-icon>
						<template #title>
							<div :class="classes.badgeIcon">
								<span
									v-for="score in selectedSentiment.values()"
									:key="score"
									:class="classes.iconBorder"
									:style="`color: ${getMetricIconColor(score)}`"
								>
									<ui-icon
										:id="IconIds.Sentiment"
										width="16"
										height="16"
									/>
								</span>
