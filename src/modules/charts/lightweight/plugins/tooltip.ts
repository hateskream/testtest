...
import { bitmapPositionsLine, timeToDate } from '../model';
...
class TooltipCrosshairLinePaneRenderer implements IPrimitivePaneRenderer {
	...
	draw(target: CanvasRenderingTarget2D) {
		...
		target.useBitmapCoordinateSpace(scope => {
			...
			const crosshairPos = bitmapPositionsLine(
				this._data.x,
...
export class TooltipPrimitive implements ISeriesPrimitive<Time> {
	...
	private _onMouseMove(param: MouseEventParams) {
		...
		const [date, time] = formatTime(param.time);
